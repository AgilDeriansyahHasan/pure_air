...
class _AdminValidasiDataPageState extends State<AdminValidasiDataPage> {
  ...
  Future<void> ambilData() async {
    ...
    await http.post(
      Uri.parse("${ApiService.baseUrl}/validasi.php"),
  ...
  Future<void> updateStatus(id, status) async {
    ...
    // Jika status di-set "Valid", backend mencoba menyinkronkan
    // data ke tabel lokasi. Tampilkan hasilnya ke admin
    // (berhasil disinkronkan, atau lokasi belum terdaftar).
    try {
      final result = json.decode(response.body);
      ...
      // Response tidak berisi info sync (misalnya status bukan Valid) — aman diabaikan.
    }
  }
            ...
            onPressed: () async {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                        "\"${d["nama_lokasi"]}\" berhasil dihapus"),
                  ),
                );
              }
            },
  ...
  // ==================
  // EKSPOR DATA (status Valid)
  // ==================

  void _eksporData(dynamic d) {
    final content = '''
Lokasi   : ${d["nama_lokasi"] ?? "-"}
Waktu    : ${d["created_at"] ?? "-"}
AQI      : ${d["aqi"] ?? "-"}
PM2.5    : ${d["pm25"]} µg/m³
PM10     : ${d["pm10"]} µg/m³
...
Status   : ${d["status"]}
  ...
  // ==================
  // TANDAI PRIORITAS (status Review)
  // ==================
          ...
          ElevatedButton.icon(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                      "\"${d["nama_lokasi"]}\" ditandai sebagai prioritas"),
                ),
              );
            },
  ...
  // Data dianggap "bermasalah" jika gagal validasi otomatis ATAU
  // sudah ditolak secara manual oleh admin (status Tolak).
  ...
  Widget build(BuildContext context) {
    ...
    final pendingCount = dataList
        .where((d) => (d["status"] ?? "Pending") == "Pending").length;
    final validCount = dataList
        .where((d) => d["status"] == "Valid").length;
    final reviewGroupCount = dataList
        .where((d) => d["status"] == "Tolak" || d["status"] == "Review").length;
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 2),
                    Text("Data akan diambil berdasarkan lokasi",
      ...
      child: Column(
        ...
        children: [
          Container(
            width: 32, height: 32,
            decoration: BoxDecoration(
                color: i.soft, borderRadius: BorderRadius.circular(10)),
      ...
      child: Column(
        ...
        children: [
          Container(
            width: 28, height: 28,
            decoration:
            BoxDecoration(color: soft, borderRadius: BorderRadius.circular(8)),
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, i) {
          ...
          final count  = t == "Semua"
              ? dataList.length
              : dataList.where((d) => (d["status"] ?? "Pending") == t).length;
          ...
          Text.rich(
            TextSpan(
              ...
              children: [
                TextSpan(
                    text: unit,
                    style: const TextStyle(fontSize: 10, color: AppColors.mutedSoft)),
              ],
            ),
          ),
        ],
  ...
  // ======================================================
  // STATION CARD — tombol berubah sesuai status
  // ======================================================
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(d["created_at"]?.toString() ?? "-",
                            style: const TextStyle(
                                fontSize: 11, color: AppColors.mutedSoft)),
                      ],
                    ),
                  ],
                ),
          ...
          // ── Baris 2: AQI badge (tengah) + Status badge (kanan) ──
          const SizedBox(height: 8),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              // AQI badge kiri-tengah
  ...
  // ======================================================
  // TOMBOL AKSI — logika per status
  //
  // Pending  → Valid | Review | Tolak
  // Valid    → (sudah valid, hanya Ekspor)
  ...
  Widget _actionButtons(dynamic d, String status) {
    ...
    Widget btnEkspor = Expanded(
      child: ElevatedButton.icon(
        onPressed: () => _eksporData(d),
    ...
    // Tombol "Pulihkan" — mengembalikan data Tolak ke Pending
    // supaya bisa ditinjau ulang tanpa harus dihapus.
    ...
    switch (status) {
    // ── Valid: hanya tombol Ekspor (full width) ──────────
      case "Valid":
        return Row(children: [btnEkspor]);

    // ── Tolak: data sudah masuk ke Data Bermasalah.
    //    Di sini cukup beri opsi memulihkan; hapus permanen
    //    dilakukan dari tabel Data Bermasalah di bawah. ────
      case "Tolak":
    ...
    // ── Pending (default): Valid + Review + Tolak ────────
      default:
          ...
          return Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  flex: 2,
                  child: Text(d["nama_lokasi"]?.toString() ?? "-",
                      style: const TextStyle(
